...
function Detail() {
    ...
    if (loading){
        return <div className="p-6 box-border">Loading...</div>;
    }
    if(!post){
        return <div className="p-6 box-border">Not Found</div>;
    }
    return (

        <div className="p-6 box-border">
            <Link to={"/"} className="no-underline text-gray-600 mb-5 block">&larr;목록으로</Link>
            <h2 className="mb-4">{post.title}</h2>
            <p className={"leading-1.5 text-gray-700"}>{post.body}</p>
        </div>
    );
}
